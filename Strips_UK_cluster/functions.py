...
def get_strips_info(x):
    ...
    df_data_grp=df_data.groupby(by="componentType_code").count().reset_index().replace(' ','_',regex=True)#,inplace=True)
...
def pd_to_lineProto_write(x):
    ...
    for i in range(len(df_data_grp)):
            global data
            data="Strips_in_"+str(x)+",componentType_code="+str(loc(i,"componentType_code"))+",id="+str(loc(i,"id"))+" "+"code="+str(loc(i,"code"))
...
def alt_chart(q):
    ...
    color=alt.Color('componentType_code', legend=alt.Legend(title='Component Code')), # filter
    #shape='id', # filter
    tooltip=['_time','_value','componentType_code']
    ).properties(
    title={
      "text": "Component migration at "+str(q), 
      "subtitle": "Strips"
    },
    width=600,
    height=350
...
def upload(x):
    dp.Report(
        *x
    ).upload(name="Component migration: Strips",publicly_visible=True)
    return()
